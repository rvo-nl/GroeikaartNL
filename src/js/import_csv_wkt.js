...
function readExcelFile (fileInput) {
  ...
  // Use SheetJS library to read the excel file
  ...
  reader.onload = function (e) {
    ...
    var columnNames = Object.keys(sheetJson[0])
    // Add the column names as the first row
    dataCsv += columnNames.join('|') + '\n'
    // Iterate through the json object and add each row to the 'rows' variable
    sheetJson.forEach(function (row) {
      // Create an array to store the values for this row
      var values = []
      // Iterate through the column names and add the corresponding value for each column
      columnNames.forEach(function (column) {
        values.push(row[column])
      })
      // Add the values for this row to the 'rows' variable, separated by the '|' symbol
      dataCsv += values.join('|') + '\n'
    })
...
var projecten = L.geoCsv(null, {
  ...
  filter: function (feature, layer) {
    ...
    if (!filterString) {
      // hits += 1
      if (feature.properties.installatiejaar <= timeSlideMax) {hits += 1; return true}else {return false}
    }
    var hit = false
